...
miniToastr.init();
export default {
    name: "status-transaksi-e-purchasing_add",
    components: {
        Datepicker
    },
    data() {
        return {
            status-transaksi-e-purchasings: [],
            provinces: [],
            regencies: {},
            formstate: {},
            model: {
    ...
    methods: {
        ...
        onSubmit: function() {
            if (this.formstate.$invalid) {
              ...
              {
                axios.post("/status-transaksi-e-purchasing/store", {
                ...
                .then(response => {
                  if(response.data.type == 'success'){
                    ...
                    window.location.href = '#/admin/status-transaksi-e-purchasing/'+response.data.id
                  }
                  else{
                    miniToastr.error(response.data.message, response.data.title)
                    window.location.href = '#/admin/status-transaksi-e-purchasing/create'
                  }
                },
                window.location.href = '#/admin/status-transaksi-e-purchasing/create')
        ...
        back() {
            window.location = '#/admin/status-transaksi-e-purchasing';
        }
    },
    mounted: function() {
        axios.get("/status-transaksi-e-purchasing/create").then(response => {
